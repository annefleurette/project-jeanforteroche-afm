    ...
    { // Si le bouton Enregistrer est choisi
        ...
        if(isset($_POST['number']) AND isset($_POST['title']) AND isset($_POST['content']))
        {
            ...
            if (empty($episode_unitary_published))
            {
                ...
                $req_add_episode = $episodeManager->addEpisodeInprogress($_POST['number'], $_POST['title'], $_POST['content']);
        ...
        if(isset($_POST['number']) AND isset($_POST['title']) AND isset($_POST['content']))
        {
            ...
            { // On publie un nouvel épisode
                $req_add_episode_published = $episodeManager->addEpisodePublished($_POST['number'], $_POST['title'], $_POST['content']);
                header('Location: index.php?action=admin');
            }else{
...
function deleteEpisode()
{
    ...
    $delete_episode = $episodeManager->deleteEpisodeDb($_GET['id']);
    header('Location: index.php?action=admin');
}
...
function updateEpisode()
{
    ...
    $_GET['id'] = htmlspecialchars($_GET['id']);
    $episode_unitary = $episodeManager->getEpisode($_GET['id']);
...
function updateEpisodePost()
{
    ...
    {   // Si le bouton Enregistrer est choisi
        ...
        if (isset($_POST['number']) AND isset($_POST['title']) AND isset($_POST['content']))
        {
            ...
            if (empty($episode_unitary_published))
            {
                ...
                $update_episode_inprogress = $episodeManager->updateEpisodeInprogress($_POST['number'], $_POST['title'], $_POST['content'], $status_progress, $_GET['id']); 
                header('Location: index.php?action=admin');
            }else{
    ...
    // Si les données ont bien été saisies
        ...
        { // Cas d'un épisode enregistré que l'on va publier
            ...
            {
                // On enregistre la modification dans la base de données
                $update_episode_inprogress = $episodeManager->updateEpisodeInprogress($_POST['number'], $_POST['title'], $_POST['content'], $status_published, $_GET['id']); 
                header('Location: index.php?action=admin');
            }else{
        ...
        }elseif(!isset($_POST['number']) AND isset($_POST['title']) AND isset($_POST['content']))
        {
            ...
            $_POST['title'] = htmlspecialchars($_POST['title']);
            $_GET['id'] = htmlspecialchars($_GET['id']);
            $update_episode_published = $episodeManager->updateEpisodePublished($_POST['title'], $_POST['content'], $_GET['id']);
            header('Location: index.php?action=admin');
        }      
    }
}
...
function lookEpisode()
{
    ...
    $_GET['id'] = htmlspecialchars($_GET['id']);
    $lookepisode = $episodeManager->getEpisodeInprogress($_GET['id']);
...
function alertCommentPost()
{
    ...
    if(in_array($id_comment, $list_alert_all)) // On regarde si le commentaire est bien dans la base de données
    {
        ...
        $update_alert = $commentManager->updateCommentAlert($alert_update, $_GET['id']);
        echo '<div class="confirmation-message"><p>Le commentaire a été signalé à l\'administration</p></div>';
        // On retourne sur l'épisode
        $exe_back_episode = $episodeManager->getEpisodeNumberBack($_GET['id']);
...
function deleteComment()
{
    ...
    $_GET['id'] = htmlspecialchars($_GET['id']);
    $delete_comment = $commentManager->deleteCommentDb($_GET['id']);
    header('Location: index.php?action=admin');
}